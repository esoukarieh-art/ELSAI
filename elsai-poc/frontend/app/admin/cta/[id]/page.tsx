...
import { deleteCTA, getCTA, updateCTA, type CTARow } from "@/lib/admin/ctaApi";
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-elsai-pin font-serif text-2xl">Éditer CTA {row?.key}</h1>
        </div>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={handleDelete}
            className="rounded-organic border-elsai-rose text-elsai-rose hover:bg-elsai-rose border px-3 py-1.5 text-sm hover:text-white"
          >
            Désactiver
          </button>
          <button
      ...
      {savedAt && <p className="mb-3 text-xs text-emerald-700">Enregistré à {savedAt}</p>}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <label className="block text-sm">
              ...
              <select
                ...
                className="rounded-organic w-full border border-slate-300 px-2 py-1.5"
              >
                {AUDIENCES.map((a) => (
        ...
        <div className="rounded-organic border-elsai-pin/15 bg-elsai-cream/40 border p-4">
          <h2 className="mb-3 text-xs text-slate-500 uppercase">Preview (client)</h2>
          {PreviewComponent && parsedProps ? (
            <PreviewComponent {...parsedProps} audience={audience} variant={variant} />
          ) : (
            <p className="text-sm text-slate-500">
              Preview indisponible (
              {!PreviewComponent ? "composant inconnu" : "props JSON invalides"}).
            </p>
          )}
        </div>
      </div>
