...
def _count(db: DBSession, event_type: str) -> int:
    return db.query(func.count(MetricEvent.id)).filter(MetricEvent.event_type == event_type).scalar() or 0
...
def metrics(db: DBSession = Depends(get_db)) -> DashboardMetrics:
    ...
    return DashboardMetrics(
        ...
        profile_breakdown={profile: count for profile, count in profile_rows},
    )
